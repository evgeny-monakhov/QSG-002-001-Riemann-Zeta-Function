...
class Config:
    ...
    NUMBER_TYPE: str = "integers"  # "integers", "primes", "3n+1", "golden", "random", "3n+2", "gaussian"
...
# =========================
def main() -> None:
    print("=== GAUSSIAN ZETA EXPERIMENT (Synthesis) ===")
    print("Method: Using YOUR engine to compute Z_Gauss = Z_Riemann * Z_Chi4")

    t = np.linspace(CFG.T_MIN, CFG.T_MAX, CFG.N_T, dtype=np.float64)

    # --- ШАГ 1: Риманов компонент ---
    print(f"[1/3] Engine: Computing Riemann Base (1, 2, 3...)...")
    n_riemann = np.arange(1, CFG.NMAX, dtype=np.float64)
    lam_r = build_vase_spectrum(n_riemann, use_2pi=CFG.USE_2PI)
    Z_riemann, _ = compute_Z_profile_integer_flow(t, lam_r, CFG.U, CFG.ALPHA, CFG.CHUNK_SIZE)

    # --- ШАГ 2: Дирихле компонент ---
    print(f"[2/3] Engine: Computing Chi-4 Part A (1, 5, 9...)...")
    ...
    print(f"[3/3] Engine: Computing Chi-4 Part B (3, 7, 11...)...")
    ...
    Z_chi4 = Z_plus - Z_minus

    # --- ШАГ 3: СИНТЕЗ ---
    print(f"[synthesis] Multiplying Z_Riemann * Z_Chi4...")
    Z_total = Z_riemann * Z_chi4
    P_total = np.abs(Z_total)

    # --- АНАЛИЗ ---
    print(f"[diag] Detecting minima...")
    minima_t = extract_profile_minima(t, P_total, refine=True)

    print(f"\n[RESULTS] Found {minima_t.size} combined zeros (Gaussian Spectrum).")
    print("Listing found zeros (check for mix of Riemann 14.1, 21.0 and Dirichlet 6.0, 10.2):")

    for i, z in enumerate(minima_t):
        label = " "
        if any(abs(z - r) < 0.2 for r in [14.13, 21.02, 25.01, 30.42]):
            label = "(Riemann)"
        elif any(abs(z - l) < 0.2 for l in [6.02, 10.24, 12.98, 16.34]):
            label = "(New/L)"
        print(f"  #{i + 1:02d}: {z:.5f} {label}")
    ...
    # Тени (прозрачные)
    ax1.plot(t, np.abs(Z_riemann), color='gray', linewidth=0.5, alpha=0.3)
    ax1.plot(t, np.abs(Z_chi4), color='green', linewidth=0.5, alpha=0.3)

    # Главная линия (Гаусс)
    ax1.plot(t, P_total, linewidth=1.2, color='blue', label=r"$|\zeta_{Gaussian}|$")
    ax1.set_title(f"Gaussian Zeta Spectrum (Riemann + Dirichlet)")
    ax1.set_xlabel("t")
    ax1.set_ylabel("|Z|")
    ...
    print(f"[plot] Marking zeros on graph...")

    # ИСПРАВЛЕНИЕ: Вместо сложной функции используем простую vlines
    # Рисуем просто красные пунктиры без текста
    ax1.vlines(minima_t, 0, y_top, colors="red", linestyles="--", linewidth=0.8, alpha=0.5)

    # А текст и точки добавляем вручную здесь
    y_text_pos = -0.02 * y_top
    for i, r in enumerate(minima_t):
        ax1.plot(r, 0, marker="o", color="red", markersize=4)
        # Сдвигаем каждый второй лейбл чуть ниже, чтобы не слипались
        offset = y_text_pos if i % 2 == 0 else y_text_pos * 2.5
        ax1.text(
            r, offset, f"{r:.2f}",
    ...
    ax2.plot(Z_total.real, Z_total.imag, linewidth=0.5, color='darkblue', alpha=0.8)

    ax2.axhline(0, color='black', linewidth=1)
    ax2.axvline(0, color='black', linewidth=1)
    ax2.plot(0, 0, marker='+', color='red', markersize=15, markeredgewidth=2, label="ZERO")

    ax2.set_title("Gaussian Phase Spiral")
    ax2.axis('equal')
