...
def auto_tune_config(t_center: float):
    ...
    effort_factor = 10.0  # 10.0 - баланс
...
class Config:
    ...
    NUMBER_TYPE: str = "gue"  # "integers", "primes", "3n+1", "golden", "random", "gue"
...
def generate_numbers(M: int, N: int, number_type: str) -> np.ndarray:
    ...
    elif number_type == "gue":
        # === GAUSSIAN UNITARY ENSEMBLE (GUE) ===
        # Это "Святой Грааль" квантового хаоса.
        # Мы генерируем случайную эрмитову матрицу и берем её собственные числа.

        # Размер матрицы. Чем больше, тем точнее статистика, но медленнее.
        # Для NMAX=300 нам нужна матрица хотя бы 500x500
        matrix_size = N * 2

        # 1. Генерируем случайную комплексную матрицу (Re и Im - нормальное распределение)
        A = np.random.randn(matrix_size, matrix_size) + 1j * np.random.randn(matrix_size, matrix_size)

        # 2. Делаем её Эрмитовой (H = (A + A*)/2)
        # Это гарантирует, что собственные числа будут ВЕЩЕСТВЕННЫМИ (как энергии)
        H = (A + A.conj().T) / 2

        # 3. Находим собственные числа (Energies)
        evals = np.linalg.eigvalsh(H)

        # 4. "Разворачивание спектра" (Unfolding)
        # У случайных матриц уровни сгущаются в центре (Закон полукруга Вигнера).
        # Нам нужно их растянуть, чтобы среднее расстояние было = 1 (как у целых чисел).
        # Берем только центральную часть спектра, где плотность равномерна.

        center_idx = matrix_size // 2
        # Берем N штук из центра
        raw_e = evals[center_idx: center_idx + N]

        # Нормируем, чтобы первое число было примерно M, а шаг примерно 1
        # Это "подгонка" под масштаб Римана, чтобы программа увидела частоты
        raw_e = raw_e - raw_e[0]  # Сдвигаем к 0
        scale_factor = 1.0 / np.mean(np.diff(raw_e))  # Средний шаг делаем 1.0

        return (raw_e * scale_factor) + M
...
# =========================
def main() -> None:
    # Генерация чисел в зависимости от конфигурации
    n = generate_numbers(CFG.M, CFG.NMAX, CFG.NUMBER_TYPE)
    lam = build_vase_spectrum(n, use_2pi=CFG.USE_2PI)

    print("=== QSSC / Integer Chaos (Vase) run ===")
    print(
        f"Config: M={CFG.M}, NMAX={CFG.NMAX}, NUMBER_TYPE={CFG.NUMBER_TYPE}, ALPHA={CFG.ALPHA:.1e}, T=[{CFG.T_MIN}, {CFG.T_MAX}]")

    t = np.linspace(CFG.T_MIN, CFG.T_MAX, CFG.N_T, dtype=np.float64)

    # Вычисляем Z профиль
    # ВАЖНО: Мы сохраняем 'Z' (комплексное число), оно нужно для правого графика!
    Z, P = compute_Z_profile_integer_flow(
        t=t,
        lam=lam,
        u=CFG.U,
        alpha=CFG.ALPHA,
    ...
    # 3. Ищем минимумы
    print(f"[diag] Detecting exact local minima of |Z(t)| profile...")
    minima_t = extract_profile_minima(t, P, refine=True)
    print(f"[diag] Found {minima_t.size} local minima.")

    # 4. Сравниваем с библиотечными нулями
    zeros_lib = get_riemann_zeros_in_range(CFG.T_MIN, CFG.T_MAX, dps=CFG.ZETA_DPS)
    compare_library_zeros_to_minima(zeros_lib=zeros_lib, minima_t=minima_t, tol=CFG.TOL, decimals=CFG.PRINT_DECIMALS,
                                    show_worst_k=CFG.SHOW_WORST_K)

    # =========================
    #      VISUALIZATION
    # =========================
    print(f"[plot] Building Phase Geometry...")

    # Создаем широкую фигуру для двух графиков
    fig = plt.figure(figsize=(14, 6))

    # --- ГРАФИК 1 (СЛЕВА): Профиль |Z(t)| (Твой старый график) ---
    ax1 = fig.add_subplot(1, 2, 1)
    ax1.plot(t, P, linewidth=1.0, color='blue', label=r"$|Z(t)|$")
    ax1.set_title(f"Amplitude: {CFG.NUMBER_TYPE}")
    ax1.set_xlabel("t")
    ax1.set_ylabel(r"$|\mathcal{Z}|$")
    ax1.grid(True, which="both", linestyle="--", alpha=0.5)

    # Рисуем нули Римана (серые линии)
    y_top = float(np.max(P))
    plot_vlines_with_labels(
        ax1, xs=zeros_lib, y_top=y_top, fmt="{:.2f}",
        color="gray", linestyle="--", linewidth=1.0, alpha=0.5,
        fontsize=7, rotation=90
    )

    # Рисуем найденные минимумы (красные точки)
    y_text_pos = -0.02 * y_top
    for r in minima_t:
        ax1.plot(r, 0, marker="o", color="red", markersize=4)
        ax1.text(r, y_text_pos, f"{r:.1f}", color="red", fontsize=7, rotation=90, ha="center", va="top")

    # --- ГРАФИК 2 (СПРАВА): Фазовая траектория (Геометрия) ---
    ax2 = fig.add_subplot(1, 2, 2)

    # Рисуем спираль. Re(Z) - ось X, Im(Z) - ось Y.
    # Если точек слишком много (>10000), рисуем линию, иначе можно точки
    ax2.plot(Z.real, Z.imag, linewidth=0.5, color='purple', alpha=0.7)

    # РИСУЕМ МИШЕНЬ (Центр координат 0,0)
    # Если линия проходит через крестик -> это ноль Римана.
    ax2.axhline(0, color='black', linewidth=1)
    ax2.axvline(0, color='black', linewidth=1)
    ax2.plot(0, 0, marker='+', color='red', markersize=15, markeredgewidth=2, label="ZERO (Target)")

    ax2.set_title("Phase Trajectory (Re vs Im)")
    ax2.set_xlabel("Re(Z)")
    ax2.set_ylabel("Im(Z)")
    ax2.axis('equal')  # Делаем оси равными, чтобы круги были круглыми
